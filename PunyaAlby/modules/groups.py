...
@Client.on_message(filters.me & filters.command(["staff", "adminlist"], [".", "-", "^", "!", "?"]))
async def adminlist(client: Client, message: Message):
    ...
    for x in badmin:
        ...
        if len(teks) >= 4096:
            ...
            teks = ""
            toolong = True
    teks += "╘══「 Jumlah {} Admin 」".format(totaladmins)
    if toolong:
        await message.reply(message.chat.id, teks, reply_to_message_id=replyid)
    else:
...
async def kickdel_cmd(client: Client, message: Message):
    ...
    await Man.edit(f"<b>Berhasil menendang {len(values)} Akun Terhapus</b>")
...
async def report_admin(client: Client, message: Message):
    ...
    if message.reply_to_message:
        if text:
            ...
            teks = "{} dilaporkan ke admin.".format(
                mention_html(
...
async def cancel_spam(client: Client, message: Message):
    if not message.chat.id in spam_chats:
        return await message.edit("__Bukan Tagall.__")
    else:
        try:
            spam_chats.remove(message.chat.id)
        except:
            pass
        return await message.edit("__Berhenti.__")    
...
async def get_list_bots(client: Client, message: Message):
    ...
    for x in bots:
        teks += "│ • {}\n".format(x)
    teks += "╘══「 Jumlah {} Bot 」".format(len(bots))
    if replyid:
